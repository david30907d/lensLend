...
const myZkAddress = "NABCv7GwqfGxGn71gnpsb5tjLC6CuxJdKTBjsVQN5Ahxyk8oU3V3tYTejt9ALRz"
...
describe("Lending Protocol", function () {
  ...
  async function depositTest() {
    ...
    const Lending = await ethers.getContractFactory("Lending");
    lendingContract = await Lending.connect(signer).deploy(bobTokenAddress, zkBobDirectDepositAddress, oracleContract.address);
    ...
    const depositBalanceOfThisAddress = await lendingContract.connect(signer).bobBalances(signer.address);
    console.log("depositBalanceOfThisAddress: ", depositBalanceOfThisAddress.toString())
  ...
  describe("Lending", function () {
    ...
    describe("createLoan()", function () {
      ...
      it("Should be able to create a loan with an 0 interestRate", async function () {
        ...
        const loanAmount = ethers.utils.parseUnits("2", 18);
        await lendingContract.connect(signer).createLoan(loanAmount, 0, stringToBytes(myZkAddress));
